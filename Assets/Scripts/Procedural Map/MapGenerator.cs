...
namespace SVS.ChessMaze
{
    public class MapGenerator : MonoBehaviour
    {
        ...
        private void Start()
        {
            if (mapAnchor != null)
            {
                mapVisualizer.cellSize = cellSize;

                Vector3 origin = mapAnchor.position;
                origin.x -= (width * cellSize) * 0.5f;
                origin.z -= (length * cellSize) * 0.5f;
                origin.y = mapAnchor.position.y;

                mapVisualizer.mapOrigin = origin;
            }
